<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:74:"G:\php\wamp64\www\tp\public/../application/index\view\user\basic_info.html";i:1505047468;}*/ ?>
<!DOCTYPE html>
		...
		<script>  
		  ...
		  function countButton(t){
		   ...
		   function checkMail(){
				...
				$.ajax({
					url:"../mail/doAction.php?act=reg",
					data:{'mail':mail},
					type:"POST",
					dataType:"Json", 
				});
				mui.toast('邮件已发送，提交后去激活');
			}
		</script>  
		<script type="text/javascript">
			var realname;
			...
			document.getElementById("send").addEventListener('tap', function() {
				...
				mui.confirm('确认信息正确吗',classname+':'+realname , btnArray, function(e) {
					if (e.index == 1) {
						...
						$.ajax({
							url:"edit_basic",
							data:{'schoolid':schoolid,'phone':phone,'mail':mail,'method':method},
							type:"POST",
							dataType:"json", 
						});
						//location='../tcpdf/examples/pdf.php';
					} else {
